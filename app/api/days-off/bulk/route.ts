...
export async function POST(request: NextRequest) {
  ...
  const { agency, year, month, days } = body;

  if (!agency || !year || !month || !days?.length) {
    return NextResponse.json({ error: 'agency, year, month, days are required' }, { status: 400 });
  }
  ...
  for (const employee of agencyEmployees) {
    ...
    const off = Array.from(new Set([...(existing?.off ?? []), ...days])).sort((a, b) => a - b);
  ...
  return NextResponse.json({ updated: agencyEmployees.length, days });
}
